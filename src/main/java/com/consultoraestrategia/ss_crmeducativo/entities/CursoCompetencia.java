package com.consultoraestrategia.ss_crmeducativo.entities;

import com.consultoraestrategia.ss_crmeducativo.lib.AppDatabase;
import com.raizlabs.android.dbflow.annotation.Column;
import com.raizlabs.android.dbflow.annotation.PrimaryKey;
import com.raizlabs.android.dbflow.annotation.Table;
import com.raizlabs.android.dbflow.annotation.Unique;
import com.raizlabs.android.dbflow.structure.BaseModel;

/**
 * Created by irvinmarin on 13/04/2017.
 */

@Table(database = AppDatabase.class)
public class CursoCompetencia extends BaseModel {
    @Unique
    @PrimaryKey
    private int cursoCompetenciaId;
    @Column
    private int cursoId;
    @Column
    private int competenciaId;

    public CursoCompetencia() {
    }

    public CursoCompetencia(int cursoCompetenciaId, int cursoId, int competenciaId) {
        this.cursoCompetenciaId = cursoCompetenciaId;
        this.cursoId = cursoId;
        this.competenciaId = competenciaId;
    }

    public int getCursoCompetenciaId() {
        return cursoCompetenciaId;
    }

    public void setCursoCompetenciaId(int cursoCompetenciaId) {
        this.cursoCompetenciaId = cursoCompetenciaId;
    }

    public int getCursoId() {
        return cursoId;
    }

    public void setCursoId(int cursoId) {
        this.cursoId = cursoId;
    }

    public int getCompetenciaId() {
        return competenciaId;
    }

    public void setCompetenciaId(int competenciaId) {
        this.competenciaId = competenciaId;
    }
}
